...
import { S007Service } from './services/s007service';
import { AgencyDto, ChildMemo, DeliveryInfoChildDto, DeliveryInfoProductDto, S007SeachDto } from './models/s007Dto';
...
export class S007Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.s007Service.init().subscribe({
      next: (response) => {
    ...
    if (this.temporarySaveInfoDto) {
      ...
      this.s007Service.seachTemporarySaveMast(dto, this.temporarySaveInfoDto).subscribe({
        next: (response) => {
          if (response.success) {
            this.allData = response.data.request_info_list;
            this.products = [...this.allData];
            this.memosAll = response.data.memo_data
            this.loadingService.hide();
          } else {
  ...
  onSeachRequest(): void {
    ...
    if (!dto.agency) {
      ...
      // range_name: '1W',
    this.s007Service.seachRequest(dto).subscribe({
      next: (response) => {
        if (response.success) {
          this.allData = response.data.request_info_list;
          this.products = [...this.allData];
  ...
  onTemporarySave() {
    ...
    this.temporaryName = this.registrationForm.get('month')?.value + '月' +  this.registrationForm.get('week')?.value + '分の請求一時保存'
    ...
    temporarySave($event:any) {
      ...
      this.s007Service.temporarySave(dto, this.products, exeName, this.temporarySaveInfoDto).subscribe({
        next: (response) => {
          if (response.success) {
            this.messageService.add({
      ...
      // 遍历所有产品
        ...
        if (input_check) {
          const dto: S007SeachDto = this.registrationForm.value
          this.s007Service.pdf(dto, this.products).subscribe({
            next: (response) => {
              if (response.success) {
                this.messageService.add({
                  severity: 'success', // 成功提示
