...
import { S024Service } from './services/s024service';
...
export class S024Component implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.initForm();
        this.s024Service.init().subscribe({
            next: (response) => {
    ...
    initForm(){
        ...
        (this.s024Form.get('filePath') as any).label = 'ファイル';
        this.addProduct();
    }

    // get品目 FormArray
    get products(): FormArray {
        return this.s024Form.get('products') as FormArray;
    }
    ...
    calculateTotalAmount(): void {
        ...
        this.s024Form.get('totalAmount')?.setValue(total.toFixed(0));
    }
    ...
    showDialog() {
        this.s024Form.get('filePath')?.setErrors(null);
        if (this.s024Form.get('filePath')?.value === "") {
            this.s024Form.get('filePath')?.setErrors({ yesFile: true });
        }

        this.s024Form.markAllAsTouched();
        this.s024Form.updateValueAndValidity();

        if (this.s024Form.invalid) {
            return;
        }

        const formValue = this.s024Form.value;
        this.s024Dto = {
    ...
    executeCreation() {
        this.s024Form.get('filePath')?.setErrors(null);
        if (this.s024Form.get('filePath')?.value !== "") {
            this.s024Form.get('filePath')?.setErrors({ noFile: true });
        }
        this.s024Form.markAllAsTouched();
        this.s024Form.updateValueAndValidity();
        ...
        this.s024Service.executeCreation(dto).subscribe({
            next: (response) => {
                this.loadingService.hide();
                if (response.success) {
    ...
    onSave() {
        ...
        this.loadingService.show();
        this.s024Service.save(this.s024Dto).subscribe({
            next: (response) => {
                this.loadingService.hide();
                this.router.navigate(['/docs/s002']);
            },
    ...
    onAgentChange(agent: any): void {
        ...
        if (agentId === 0) {
            this.s024Form.get('agent')?.setErrors({ required: true });  
        }
        this.stores = this.storesMap[agentId] || [];
    }

    onDragOver(event: DragEvent): void {
        // ドラッグ中のデフォルトの動作（例：ファイルの開き動作）を防止します。
    ...
    onFileDrop(event: DragEvent): void {
        ...
        if (event.dataTransfer?.files?.length) {
            ...
            const fileControl = this.s024Form.get('file');
            ...
            this.processFile(this.file)
                .then((fileData) => {
                    // フォームにファイルパスを設定します。
                    this.s024Form.patchValue({ filePath: this.file?.name });
    
                    // フォームにファイルの内容を設定します。
                    this.s024Form.patchValue({ file: fileData });
                })
                .catch((error) => {
    ...
    removeFile(): void {
        ...
        // フォームのファイルパスを空にします。
        this.s024Form.patchValue({ filePath: "" });
        this.s024Form.patchValue({ file: null });
    }
