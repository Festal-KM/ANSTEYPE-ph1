...
import { S008Service } from './services/s008service';
...
@Component({
  selector: 'app-s008',
  templateUrl: './s008.component.html',
  styleUrl: './s008.component.scss'
})

export class S008Component implements OnInit {
  ...
  loadInitialData(): void {
    ...
    this.s008Service.getData().subscribe({
      next: (response) => {
  ...
  onSubmit() {
    ...
    if (this.quotationForm.valid) {
      ...
      this.s008Service.add(dto).subscribe({
        next: (response) => {
          this.loadingService.hide();
