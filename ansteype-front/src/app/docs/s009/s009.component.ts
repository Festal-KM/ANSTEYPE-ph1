...
import { DeliveryInfoProduct, QuotationClaimCreationInfoDto } from './models/s009Dto';
import { LoadingService } from '../../add-ins/service/loading.service';
import { S009Service } from './services/s009service';
...
@Component({
  selector: 'app-s009',
  templateUrl: './s009.component.html',
  styleUrl: './s009.component.scss'
})
export class S009Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.s009Service.getSendMailInfoText(this.products).subscribe({
      next: (response) => {
  ...
  // 使用 Set 存储所有展开的行
  expandedRows: Set<DeliveryInfoProduct> = new Set();
  ...
  selectRow(rowIndex: any, pdf: any, child: any, product: any, productRowIndex:any, childIndex: any) {
    ...
    this.s009Service.getSendMailInfo(child, pdf).subscribe({
      next: (response) => {
  ...
  saveEmailInof() {
    ...
    // this.s009Service.saveSendMailInfo(this.registrationForm.value).subscribe({
    //   next: (response) => {
    //     this.loadingService.hide();
    //   },
    //   error: (error) => {
  ...
  sendEmail() {
    ...
    this.s009Service.sendMail(this.products).subscribe({
      next: (response) => {
