...
import { S026Service } from './services/s026service';
...
@Component({
    selector: 'app-s026',
    templateUrl: './s026.component.html',
    styleUrl: './s026.component.scss'
})
export class S026Component {
      ...
      private fb: FormBuilder)
   {
    ...
    this.s026Service.init().subscribe({
      next: (response) => {
     ...
     */
    onSeachMast(): void {
      const dto: S026SeachDto = this.registrationForm.value
      if (!this.registrationForm.get('year')?.value || !this.registrationForm.get('month')?.value ) {
        this.messageService.add({
          severity: 'error', // 错误类型
          summary: 'エラー',   // 标题
          detail: '対象年と対象月を入力してください', // 详细信息
        });
        return;
      }
      
      this.loadingService.show();
          this.s026Service.seach(dto).subscribe({
            next: (response) => {
    ...
    previewPageShow() {
      ...
      this.router.navigateByUrl('/docs/s009', {
        state: {
          mode: 'edit',
          selectedDTOs: this.selectedItems, // 传递数据集合
        },
      });
    }
  ...
  addRow(product: any) {
    if (!product.store_address_settings || product.store_address_settings.length === 0) {
      product.store_address_settings = [
        { value: '' }, // 第一条
        { value: '' }, // 第二条
      ];
    } else {
  ...
  addAddressRow(index: number) {
    if (this.products[index].store_address_settings.length=0){
        this.products[index].store_address_settings.push({ value: this.placeholderAddressA.value });
